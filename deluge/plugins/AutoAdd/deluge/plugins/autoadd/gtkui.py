...
class OptionsDialog(object):
    ...
    def load_options(self, options):
        ...
        # FIXME AttributeError: 'gi.FunctionInfo' object has no attribute 'set_text'
        ...
        for spin_id in self.spin_ids + self.spin_int_ids:
            self.main_builder.get_object(spin_id).set_value(options.get(spin_id, 0))
            self.main_builder.get_object(spin_id + "_toggle").set_active(options.get(spin_id + "_toggle", False))
        for chk_id in self.chk_ids:
            self.main_builder.get_object(chk_id).set_active(bool(options.get(chk_id, True)))
            self.main_builder.get_object(chk_id + "_toggle").set_active(options.get(chk_id + "_toggle", False))
    ...
    def set_sensitive(self):
        ...
        for maintoggle in maintoggles:
            self.on_toggle_toggled(self.main_builder.get_object(maintoggle + "_toggle"))
    ...
    def generate_opts(self):
        ...
        options['label'] = self.labels[
            self.main_builder.get_object('label').get_active()][0]
        ...
        for spin_id in self.spin_ids:
            options[spin_id] = self.main_builder.get_object(spin_id).get_value()
            options[spin_id + '_toggle'] = self.main_builder.get_object(spin_id + '_toggle').get_active()
        for spin_int_id in self.spin_int_ids:
            options[spin_int_id] = self.main_builder.get_object(spin_int_id).get_value_as_int()
            options[spin_int_id + '_toggle'] = self.main_builder.get_object(spin_int_id + '_toggle').get_active()
        for chk_id in self.chk_ids:
            options[chk_id] = self.main_builder.get_object(chk_id).get_active()
            options[chk_id + '_toggle'] = self.main_builder.get_object(chk_id + '_toggle').get_active()
